...
require_fixed 'let maximumCoordinate = centerCoordinate' 'Sources/UI/WorldMapViewController.swift' 'axis picker maximum must stop at the current rendered X/Z plane because projection only extends toward the negative axis'
...
require_fixed 'if let index = result.blocks.firstIndex(where: { !$0.primaryState.isAir }) {' 'Sources/UI/BlockAxisPickerViewController.swift' 'X/Z picker must default to the largest-coordinate non-air block in its descending list'
