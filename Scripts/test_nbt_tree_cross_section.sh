...
require_fixed 'case 1: return CrossSectionExportAngle.allCases.count' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export angle row must expose all six x+/x-/y+/y-/z+/z- directions'
require_fixed 'negativeInfinityButton.setTitle("-∞", for: .normal)' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export range must provide a -infinity shortcut'
require_fixed 'positiveInfinityButton.setTitle("+∞", for: .normal)' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export range must provide a +infinity shortcut'
...
require_fixed '同时选择四个无穷边界时等效于导出当前维度全部已加载区域' 'Sources/UI/MapExportOptionsViewController.swift' 'cross-section export UI must explain the all-infinity loaded-dimension shortcut'
