...
require_fixed 'cg.setLineWidth(max(0.15, CGFloat(rasterPixelsPerSample) * 0.15))' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z SubChunk grid must use a thinner Y-map-like line width'
...
require_fixed 'pixelsPerBlock: 2' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must request higher-resolution section rasterization'
