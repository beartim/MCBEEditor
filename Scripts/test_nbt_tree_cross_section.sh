...
require_fixed 'let maximumCoordinate = currentMode == .xray' 'Sources/UI/WorldMapViewController.swift' 'mineral picker must cap its read range at the current X/Z plane'
require_fixed '? centerCoordinate' 'Sources/UI/WorldMapViewController.swift' 'mineral picker must read current X/Z through current X/Z-128 only'
...
require_fixed 'let maximumProjectionCoordinate = axis == .x ? fixedX : fixedZ' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z projected block lookup must explicitly start from the largest visible axis coordinate'
...
require_fixed 'through: minimumProjectionCoordinate' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z projection must continue through the full 128-block negative-axis slab'
...
require_fixed 'preferHighlightedOre: Bool = false' 'Sources/UI/BlockAxisPickerViewController.swift' 'X/Z picker must support a mineral-specific automatic selection mode'
require_fixed 'BedrockBlockIdentifier.isHighlightedOre(block.primaryState.name)' 'Sources/UI/BlockAxisPickerViewController.swift' 'mineral view must default to the largest highlighted ore coordinate rather than the largest arbitrary non-air block'
require_fixed 'preferHighlightedOre: self.currentMode == .xray' 'Sources/UI/WorldMapViewController.swift' 'X/Z taps must enable mineral-specific automatic selection in xray mode'
require_fixed 'cg.setShouldAntialias(true)' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z grid stroke must be antialiased so its visible center, not its outer edge, aligns to the exact block boundary'
require_fixed 'visible stroke is centered on that edge' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z grid alignment fix must keep the path centered on the world block edge'
...
require_fixed 'ungeneratedDisplay: verticalSlice ? .air : .transparent' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must default missing SubChunks to ordinary air while Y export keeps its existing transparent default'
require_fixed 'airUngeneratedSubChunks: layers.ungeneratedDisplay == .air' 'Sources/UI/WorldMapViewController.swift' 'X/Z air export must explicitly replace missing selected-plane SubChunks with air instead of leaking projected terrain from behind'
require_fixed 'guard showUngeneratedSubChunks || transparentUngeneratedSubChunks || airUngeneratedSubChunks else { return [] }' 'Sources/Chunk/BedrockCrossSection.swift' 'air export must compute exact missing SubChunk rectangles'
...
require_fixed 'let projectionNegativeDistance: Int64 = mode == .xray ? 128 : 127' 'Sources/UI/WorldMapViewController.swift' 'xray all-loaded export must include current X/Z through current X/Z-128 while other modes keep the established 128-cell slab'
require_fixed 'let projectionNegativeDistance = mode == .xray ? rayDepth : max(0, rayDepth - 1)' 'Sources/Chunk/BedrockCrossSection.swift' 'xray sampling must include the current coordinate and the -128 endpoint'
