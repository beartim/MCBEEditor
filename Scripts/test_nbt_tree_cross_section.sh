...
require_fixed 'private let crossSectionSelectionHalfRange: Int64 = 128' 'Sources/UI/WorldMapViewController.swift' 'X/Z negative-axis selection reach must remain 128 blocks'
...
require_fixed 'let maximumCoordinate = centerCoordinate + crossSectionSelectionHalfRange' 'Sources/UI/WorldMapViewController.swift' 'axis picker manual range must remain current X/Z +128 even though projection only extends toward the negative axis'
require_fixed 'automaticSelectionMaximumCoordinate: centerCoordinate' 'Sources/UI/WorldMapViewController.swift' 'axis picker automatic initial row must be capped at the rendered plane'
...
require_fixed 'else if showUngeneratedSubChunks || transparentUngeneratedSubChunks {' 'Sources/Chunk/BedrockCrossSection.swift' 'showing missing X/Z SubChunks must shrink block projection to the section-plane chunk instead of the normal 128-block depth'
require_fixed 'defaultProjectionCoordinateRange = minimumChunkCoordinate...(minimumChunkCoordinate + 15)' 'Sources/Chunk/BedrockCrossSection.swift' 'missing X/Z display must project only the 16-block chunk containing the section plane'
require_fixed 'let exactUngeneratedRects = ungeneratedPlaneSubChunkRects()' 'Sources/Chunk/BedrockCrossSection.swift' 'missing-section texture must be based on exact selected-plane SubChunk cells'
...
require_fixed 'let maximumProjectionCoordinate = defaultProjectionCoordinateRange.upperBound' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z projected block lookup must use the configured upper projection bound'
require_fixed 'for coordinate in stride(' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z block rendering must inspect the full projection ray in descending coordinate order'
require_fixed 'case .negativeToPositive:' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z projection must support reverse negative-to-positive viewing as well as the default direction'
...
require_fixed 'hasSubChunk: subChunk != nil' 'Sources/Chunk/BedrockCrossSection.swift' 'missing SubChunk records must remain genuinely ungenerated even when the parent chunk exists'
forbid_fixed 'generatedOrStored = subChunk != nil || generatedChunkPositions.contains(position)' 'Sources/Chunk/BedrockCrossSection.swift' 'chunk-level generated state must not turn missing sky SubChunks into ordinary air'
forbid_fixed 'if let generatedFallback = generatedFallback {' 'Sources/Chunk/BedrockCrossSection.swift' 'projected generated-air fallback must not hide genuinely missing SubChunks'
require_fixed 'func ungeneratedPlaneSubChunkRects() -> [CGRect]' 'Sources/Chunk/BedrockCrossSection.swift' 'missing X/Z texture must be clipped by exact world SubChunk cells rather than sampled pixels'
require_fixed '!planeHasSubChunk(horizontal: clippedHorizontalStart, y: clippedYStart)' 'Sources/Chunk/BedrockCrossSection.swift' 'missing X/Z cells must be decided from the actual selected plane SubChunk record'
...
require_fixed 'let worldAnchorIntercept = CGFloat(' 'Sources/Chunk/BedrockCrossSection.swift' 'missing X/Z hatch phase must be anchored to world coordinates instead of viewport pixels'
require_fixed 'if drawSubChunkGrid {' 'Sources/Chunk/BedrockCrossSection.swift' 'subchunk grid must remain a separate final overlay above missing-section texture'
require_fixed 'guard coordinate >= automaticLowerBound, coordinate <= automaticUpperBound else { return false }' 'Sources/UI/BlockAxisPickerViewController.swift' 'X/Z picker automatic selection must stay inside the rendered negative-axis window while retaining manual rows outside it'
require_fixed 'BedrockBlockIdentifier.isHighlightedOre(block.primaryState.name)' 'Sources/UI/BlockAxisPickerViewController.swift' 'X/Z X-ray picker must default to the largest highlighted ore in the projection window'
require_fixed 'return !block.primaryState.isAir' 'Sources/UI/BlockAxisPickerViewController.swift' 'non-X-ray X/Z picker must continue defaulting to the largest non-air block'
require_fixed 'let gridLineWidth = max(0.15, worldToRaster * 0.15)' 'Sources/Chunk/BedrockCrossSection.swift' 'X/Z SubChunk grid width must keep the per-block scale formula'
require_fixed 'x: x - gridLineWidth * 0.5' 'Sources/Chunk/BedrockCrossSection.swift' 'vertical SubChunk grid line must be geometrically centered on the exact block boundary'
require_fixed 'y: y - gridLineWidth * 0.5' 'Sources/Chunk/BedrockCrossSection.swift' 'horizontal SubChunk grid line must be geometrically centered on the exact block boundary'
...
require_fixed 'override func numberOfSections(in tableView: UITableView) -> Int { isCrossSection ? 4 : 3 }' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export options must expose range, angle, layer and ungenerated-display sections'
require_fixed 'case 1: return 1' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export angle section must use one six-way selector row'
require_fixed 'private let control = UISegmentedControl(items: CrossSectionExportAngle.allCases.map(\.displayName))' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export angles must be presented as one six-segment selector'
require_fixed 'makeCoordinateGroup(title: "x1", field: minimumXField, infinityButton: minimumXInfinityButton, infinityTitle: "-∞"' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export x1 must have its own -infinity shortcut'
require_fixed 'makeCoordinateGroup(title: "z1", field: minimumZField, infinityButton: minimumZInfinityButton, infinityTitle: "-∞"' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export z1 must have its own -infinity shortcut'
require_fixed 'makeCoordinateGroup(title: "x2", field: maximumXField, infinityButton: maximumXInfinityButton, infinityTitle: "+∞"' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export x2 must have its own +infinity shortcut'
require_fixed 'makeCoordinateGroup(title: "z2", field: maximumZField, infinityButton: maximumZInfinityButton, infinityTitle: "+∞"' 'Sources/UI/MapExportOptionsViewController.swift' 'X/Z export z2 must have its own +infinity shortcut'
require_fixed 'infinityButton.backgroundColor = .systemBlue' 'Sources/UI/MapExportOptionsViewController.swift' 'all four infinity shortcuts must use blue rounded buttons'
require_fixed 'infinityButton.layer.cornerRadius = 8' 'Sources/UI/MapExportOptionsViewController.swift' 'all four infinity shortcuts must use rounded rectangles'
forbid_fixed 'tableView.reloadData()' 'Sources/UI/MapExportOptionsViewController.swift' 'export option taps must not reload the whole table and jump the sheet to the top'
require_fixed 'currentSliceAxis == .x ? .xPositive : .zPositive' 'Sources/UI/WorldMapViewController.swift' 'X and Z modes must default export angle to x+ and z+ respectively'
require_fixed 'projectionDirection: projectionDirection' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must forward positive/negative viewing direction to the renderer'
require_fixed 'startCrossSectionImageExport(scope: scope, layers: layers, configuration: crossSectionConfiguration)' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must forward the custom range and viewing angle into the section-image exporter'
require_fixed 'let projectionRange: ClosedRange<Int64>' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must derive its projection depth from the entered X/Z range'
...
require_fixed 'minimumX: currentStartBlockX' 'Sources/UI/WorldMapViewController.swift' 'X/Z export range must default to the current visible map X boundary'
require_fixed 'let resolvedMinimumX = request.minimumX ?? loadedMinimumX' 'Sources/UI/WorldMapViewController.swift' 'infinite X/Z export bounds must resolve to the loaded-dimension extent'
require_fixed '四个范围均设为无穷时等效于导出当前维度全部已加载区域' 'Sources/UI/MapExportOptionsViewController.swift' 'cross-section export UI must explain the all-infinity loaded-dimension shortcut'
...
require_fixed 'projectionCoordinateRange: projectionRange' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must pass the entered projection range directly to the renderer'
require_fixed 'projectionDepth: mode == .xray ? 129 : 128' 'Sources/UI/WorldMapViewController.swift' 'X/Z X-ray render must sample current X/Z through X/Z-128 inclusive'
require_fixed 'ungeneratedDisplay: verticalSlice ? .air : .transparent' 'Sources/UI/WorldMapViewController.swift' 'X/Z export must default missing SubChunks to air while Y export keeps the existing transparent default'
