...
struct CommandBlockStateSpec {
    ...
    func canRemainLegacy(layer: Int) -> Bool {
        guard layer == 0 || layer == 1,
              let block = BedrockLegacyBlockCatalog.block(forIdentifier: name), (0...255).contains(block.id),
              let data = BedrockLegacyBlockStateConverter.numericData(for: modernState(version: nil)) else { return false }
        return layer == 1 || data <= 15
    }

    func legacyState() throws -> BedrockBlockState {
        guard let block = BedrockLegacyBlockCatalog.block(forIdentifier: name), (0...255).contains(block.id) else {
            throw MCBEEditorError.unsupported("方块 \(name) 没有可用的旧版数字 ID")
        }
        guard let data = BedrockLegacyBlockStateConverter.numericData(for: modernState(version: nil)) else {
            throw MCBEEditorError.unsupported("旧版数字 ID 无法无损表示 \(name) 的 states")
        }
        return BedrockBlockState(nbt: nil, legacyID: UInt16(block.id), legacyData: data)
    }
}
...
enum WorldCommandParser {
    ...
    static func parse(_ line: String) throws -> ParsedWorldCommand {
        ...
        case "chunk":
            ...
            case "regenerate":
                guard arguments.count == 4 else { throw usageError(command) }
                return .chunk(operation: .regenerate(
                    dimension: try parseDimension(arguments[1]),
                    x: try parseChunkCoordinate(arguments[2], name: "区块 X"),
                    z: try parseChunkCoordinate(arguments[3], name: "区块 Z")
                ))
