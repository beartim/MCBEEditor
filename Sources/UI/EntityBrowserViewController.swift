...
final class EntityBrowserViewController: UIViewController, UITableViewDataSource,
  UITableViewDelegate, UISearchResultsUpdating, UITextFieldDelegate
{
  ...
  private let dimensionControl = UISegmentedControl(
    items: ["全部"] + BedrockDimension.allCases.map(\.displayName))
  ...
  private let scanQueue = DispatchQueue(
    label: "com.wzn.mcbeeditor.entity-scan", qos: .userInitiated)
  ...
  private func configureUI() {
    ...
    rectangleX0.placeholder = "X0"
    rectangleZ0.placeholder = "Z0"
    rectangleX1.placeholder = "X1"
    rectangleZ1.placeholder = "Z1"
    rectangleY0.placeholder = "Y0"
    rectangleY1.placeholder = "Y1"
    radiusX.placeholder = "X"
    radiusY.placeholder = "Y"
    radiusZ.placeholder = "Z"
    ...
    statusLabel.addGestureRecognizer(
      UITapGestureRecognizer(target: self, action: #selector(showDiagnostics)))
  ...
  private func configureRectanglePanel() {
    ...
    let first = coordinatePair(
      title: "起点", xTitle: "X0", xField: rectangleX0, zTitle: "Z0", zField: rectangleZ0)
    let second = coordinatePair(
      title: "终点", xTitle: "X1", xField: rectangleX1, zTitle: "Z1", zField: rectangleZ1)
  ...
  private func configureRadiusPanel() {
    ...
    let center = coordinatePair(
      title: "中心", xTitle: "X", xField: radiusX, zTitle: "Z", zField: radiusZ)
  ...
  private func coordinatePair(
    title: String, xTitle: String, xField: UITextField, zTitle: String, zField: UITextField
  ) -> UIStackView {
    let titleLabel = compactLabel(title)
    titleLabel.widthAnchor.constraint(equalToConstant: 34).isActive = true
    let row = UIStackView(arrangedSubviews: [
      titleLabel, compactLabel(xTitle), xField, compactLabel(zTitle), zField,
    ])
    row.axis = .horizontal
    row.spacing = 6
  ...
  private func applySelectionCoordinate(_ coordinate: WorldSelectionCoordinate) {
    ...
    if let index = BedrockDimension.allCases.firstIndex(where: {
  ...
  @objc private func createObject() {
    let kind: BedrockWorldObjectKind =
      kindControl.selectedSegmentIndex == 0 ? .entity : .blockEntity
  ...
  private func confirmDelete(_ object: BedrockWorldObject) {
    let alert = UIAlertController(
      ...
      message:
        "将从世界 LevelDB 删除“\(dataValueDisplayName(for: object))”。现代实体的 actorprefix 和 digp 引用会同步清理。此操作不可撤销。",
    ...
    scanQueue.async { [weak self] in
      ...
      do {
        ...
        DispatchQueue.main.async {
          ...
          self.scanSummary =
            "\(dimensionText)：实体 \(entityCount)，方块实体 \(blockCount)；摘要 \(result.actorDigestCount)，actor \(result.actorRecordCount)\(repaired)\(errors)"
  ...
  private func makeSpatialFilter() throws -> (EntitySpatialFilter, String) {
    ...
    case .box:
      guard let x0 = Double(rectangleX0.text ?? ""), let z0 = Double(rectangleZ0.text ?? ""),
        let x1 = Double(rectangleX1.text ?? ""), let z1 = Double(rectangleZ1.text ?? "")
      else {
        throw MCBEEditorError.malformedData("坐标区域的 X0、Z0、X1、Z1 必须是数字")
      }
      let yRange: ClosedRange<Double>?
      if rectangleYSwitch.isOn {
        guard let y0 = Double(rectangleY0.text ?? ""), let y1 = Double(rectangleY1.text ?? "")
        else {
      ...
      if let yRange {
        description = "区域 X/Z + Y=\(format(yRange.lowerBound))…\(format(yRange.upperBound))"
      } else {
        description =
    ...
    case .radius:
      guard let x = Double(radiusX.text ?? ""), let z = Double(radiusZ.text ?? ""),
        let radius = Double(radiusValue.text ?? ""), radius >= 0
      else {
      ...
      let description: String
      if let y {
        description = "XYZ 半径 \(format(radius))，中心 (\(format(x)), \(format(y)), \(format(z)))"
      } else {
        description = "XZ 半径 \(format(radius))，中心 (\(format(x)), \(format(z)))"
      }
  ...
  private func applyFilter(showErrors: Bool) {
    ...
    do {
      ...
      statusLabel.text =
        scanSummary.isEmpty
        ? error.localizedDescription : "\(scanSummary)；范围参数尚未生效：\(error.localizedDescription)"
      return
    }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    let cell =
      tableView.dequeueReusableCell(withIdentifier: "WorldObjectCell")
      ?? UITableViewCell(style: .subtitle, reuseIdentifier: "WorldObjectCell")
  ...
  func tableView(
    ...
    let duplicate = UIContextualAction(style: .normal, title: "新建副本") {
      [weak self] _, _, completion in
  ...
  func tableView(
    ...
    return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { [weak self] _ in
      ...
      var actions = [
        UIAction(title: "编辑 NBT", image: UIImage(systemName: "square.and.pencil")) {
          [weak self] _ in
      ...
      actions.append(
        UIAction(
          title: "复制为新\(object.kind.displayName)",
      ...
      if object.kind == .entity {
        actions.append(
          UIAction(title: "导出实体 NBT", image: UIImage(systemName: "square.and.arrow.up")) {
            [weak self] _ in
      ...
      if object.position != nil {
        actions.append(
          UIAction(title: "在地图中定位", image: UIImage(systemName: "map")) { [weak self] _ in
            self?.onLocate(object)
          })
      }
      actions.append(
  ...
  private func showDetails(_ object: BedrockWorldObject) {
    ...
    let alert = UIAlertController(
      title: dataValueDisplayName(for: object), message: message, preferredStyle: .actionSheet)
    alert.addAction(
      UIAlertAction(title: "编辑 NBT", style: .default) { [weak self] _ in
        self?.openEditor(object)
      })
    alert.addAction(
      UIAlertAction(title: "复制为新\(object.kind.displayName)", style: .default) { [weak self] _ in
        self?.duplicateObject(object)
      })
    if object.kind == .entity {
      alert.addAction(
        UIAlertAction(title: "导出实体 NBT", style: .default) { [weak self] _ in
          self?.exportEntity(object)
        })
    }
    alert.addAction(
      UIAlertAction(title: "删除\(object.kind.displayName)", style: .destructive) { [weak self] _ in
        self?.confirmDelete(object)
      })
    if object.position != nil {
      alert.addAction(
        UIAlertAction(title: "在地图中定位", style: .default) { [weak self] _ in self?.onLocate(object) })
    }
    alert.addAction(
  ...
  private func dataValueDisplayName(for object: BedrockWorldObject) -> String {
    ...
    {
      if let customName = object.customName?.trimmingCharacters(in: .whitespacesAndNewlines),
        !customName.isEmpty
      {
  ...
  @objc private func showDiagnostics() {
    let message =
      diagnostics.isEmpty ? "本次扫描没有记录解析错误。" : diagnostics.prefix(120).joined(separator: "\n")
