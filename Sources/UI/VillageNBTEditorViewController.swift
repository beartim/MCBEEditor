...
final class VillageNBTEditorViewController: UITableViewController, UISearchResultsUpdating {
  ...
  private let residentQueue = DispatchQueue(
    label: "com.wzn.mcbeeditor.village-residents", qos: .userInitiated)
  ...
  private func loadResidentEntities() {
    ...
    residentQueue.async { [weak self] in
      ...
      do {
        let resolution = try self.store.residentResolution(
          villageIdentifier: self.record.villageIdentifier)
  ...
  override func tableView(_ tableView: UITableView, titleForFooterInSection section: Int) -> String?
  {
    if showsResidentOptions && section == 0 {
      ...
      let unresolved =
        resolution.unresolvedUniqueIDs.isEmpty
        ? ""
    ...
    -> UITableViewCell
  {
    if showsResidentOptions && indexPath.section == 0 {
      ...
      let cell =
        tableView.dequeueReusableCell(withIdentifier: "VillageResidentOptionCell")
      ...
      cell.detailTextLabel?.text =
        isLoadingResidents ? "正在匹配 Dwellers ID（实体 UniqueID）…" : "共 \(count ?? 0) 个"
    ...
    NBTEditingUI.presentEdit(from: self, node: node) { [weak self] replacement in
      ...
      do {
        self.document.root = try NBTTreeMutation.replacingValue(
          at: node.path, in: self.document.root, with: replacement)
        self.dirty = true
        self.rebuildRows()
      } catch {
