...
final class StructureNBTEditorViewController: UITableViewController, UISearchResultsUpdating {
  ...
  @objc private func worldDidChange() {
    ...
    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
      ...
      DispatchQueue.main.async { [weak self] in
        ...
        guard let refreshed = refreshed, let document = refreshed.document else {
          self.navigationItem.prompt = "该结构记录已被命令删除或无法重新读取。"
          return
        }
    ...
    -> UITableViewCell
  {
    ...
    let cell =
      (tableView.dequeueReusableCell(withIdentifier: "StructureNBTNodeCell") as? NBTTreeCell)
      ?? NBTTreeCell(style: .default, reuseIdentifier: "StructureNBTNodeCell")
    cell.configure(
      node: node,
  ...
  @objc private func confirmSave() {
    ...
    alert.addAction(
      UIAlertAction(title: "保存", style: .destructive) { [weak self] _ in self?.performSave() })
