...
final class WorldObjectNBTEditorViewController: UITableViewController, UISearchResultsUpdating {
  ...
  private func configureNavigationItems() {
    ...
    navigationItem.rightBarButtonItems = [
      ...
      UIBarButtonItem(
        barButtonSystemItem: .trash, target: self, action: #selector(confirmDeleteObject)),
    ...
    -> UITableViewCell
  {
    ...
    let cell =
      (tableView.dequeueReusableCell(withIdentifier: "WorldObjectNBTCell") as? NBTTreeCell)
      ?? NBTTreeCell(style: .default, reuseIdentifier: "WorldObjectNBTCell")
    cell.configure(
      node: node,
  ...
  override func tableView(
    ...
    if node.value.isDirectlyEditable {
      let editAction = UIContextualAction(style: .normal, title: "修改") {
  ...
  @objc private func confirmSave() {
    ...
    alert.addAction(UIAlertAction(title: "取消", style: .cancel))
    alert.addAction(
      UIAlertAction(title: "保存", style: .destructive) { [weak self] _ in
        self?.performSave()
      })
    present(alert, animated: true)
  }
