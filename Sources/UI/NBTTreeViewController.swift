...
final class NBTTreeViewController: UITableViewController, UISearchResultsUpdating {
  ...
  private func rebuildRows() {
    ...
    if searchQuery.isEmpty {
      rows = NBTTreeRows.visibleChildren(of: root, expanded: expanded)
      navigationItem.prompt = nil
    } else {
      rows = NBTTreeRows.search(in: root, query: searchQuery)
    ...
    -> UITableViewCell
  {
    ...
    let cell =
      (tableView.dequeueReusableCell(withIdentifier: "NBTCell") as? NBTTreeCell)
      ?? NBTTreeCell(style: .default, reuseIdentifier: "NBTCell")
    cell.configure(
      node: node,
  ...
  private func edit(_ node: NBTNode) {
    ...
    NBTEditingUI.presentEdit(from: self, node: node) { [weak self] replacement in
      guard let self = self else { return }
      do {
        file.document.root = try NBTTreeMutation.replacingValue(
          at: node.path, in: file.document.root, with: replacement)
        self.levelDat = file
        self.dirty = true
        self.rebuildRows()
      } catch {
  ...
  @objc private func addToRoot() {
    ...
    NBTEditingUI.presentAddOrPaste(from: self, container: root, sourceView: view) {
  ...
  private func add(
    ...
    guard var file = levelDat else { return }
    do {
      file.document.root = try NBTTreeMutation.adding(
        value, named: name, to: path, in: file.document.root, replacingExisting: replacingExisting)
      levelDat = file
  ...
  private func rename(_ node: NBTNode, to name: String) {
    guard var file = levelDat else { return }
    do {
      file.document.root = try NBTTreeMutation.renaming(
        at: node.path, to: name, in: file.document.root)
      levelDat = file
      expanded = [[]]
      dirty = true
      rebuildRows()
        ...
        actions.append(
          UIAction(title: "增加子标签", image: UIImage(systemName: "plus")) { [weak self] _ in
            ...
            NBTEditingUI.presentAdd(
              from: self, container: node.value, sourceView: tableView.cellForRow(at: indexPath)
            ) { [weak self] name, value, replacingExisting in
              self?.add(
