...
final class StandaloneNBTEditorViewController: UITableViewController, UISearchResultsUpdating {
    ...
    private func rebuildRows() {
        ...
        if query.isEmpty {
            ...
            navigationItem.prompt = "根名称：\(document.rootName.isEmpty ? "（空）" : document.rootName)\(dirty ? " · 未保存" : "")"
        } else {
            if rootNeedsOwnRow {
                let root = NBTNode(
                    path: [],
                ...
                if matches(root) { rows.append(root) }
            }
    ...
    private func appendChildren(of value: NBTValue, path: [NBTPathComponent], depth: Int) {
        ...
        case .compound(let tags):
            for tag in tags {
                ...
                rows.append(node)
                if expanded.contains(childPath) {
                    appendChildren(of: tag.value, path: childPath, depth: depth + 1)
                }
            }
        case .list(_, let values):
            for (index, child) in values.enumerated() {
                ...
                rows.append(node)
                if expanded.contains(childPath) {
                    appendChildren(of: child, path: childPath, depth: depth + 1)
                }
            }
        default:
            break
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "StandaloneNBTNodeCell")
    ...
    @objc private func editRootName() {
        ...
        alert.addAction(UIAlertAction(title: "修改", style: .default) { [weak self, weak alert] _ in
            guard let self = self else { return }
