...
enum NBTTreeMutation {
  ...
  static func replacingValue(
    at path: [NBTPathComponent], in root: NBTValue, with replacement: NBTValue
  ) throws -> NBTValue {
    ...
    case (.compound(let name), .compound(var tags)):
      guard let index = tags.firstIndex(where: { $0.name == name }) else {
        throw MCBEEditorError.malformedData("NBT 路径不存在：\(name)")
      }
    ...
    case (.list(let index), .list(let type, var values)):
      guard values.indices.contains(index) else {
        throw MCBEEditorError.malformedData("NBT 列表索引越界：\(index)")
      }
      if tail.isEmpty, replacement.type != type {
        throw MCBEEditorError.malformedData("NBT 列表元素类型不一致")
      }
      values[index] = try replacingValue(at: tail, in: values[index], with: replacement)
      return .list(type, values)
    default:
      throw MCBEEditorError.malformedData("NBT 路径与节点类型不匹配")
    }
  }
  ...
  static func adding(
    ...
    guard let container = self.value(at: path, in: root) else {
      throw MCBEEditorError.malformedData("NBT 容器路径不存在")
    }
    let replacement = try appending(
      value, named: name, to: container, replacingExisting: replacingExisting)
  ...
  private static func appending(
    ...
    case .compound(var tags):
      ...
      guard !cleanName.isEmpty else {
        throw MCBEEditorError.malformedData("Compound 标签名称不能为空")
      }
      if let existingIndex = tags.firstIndex(where: { $0.name == cleanName }) {
        guard replacingExisting else {
          throw MCBEEditorError.malformedData("同级 Compound 已存在标签：\(cleanName)")
        }
    ...
    case .list(let elementType, var values):
      ...
      guard value.type == elementType else {
        throw MCBEEditorError.malformedData("List 只能增加 \(elementType.displayName) 元素")
      }
      values.append(value)
      return .list(elementType, values)
    default:
      throw MCBEEditorError.unsupported("只能向 Compound 或 List 增加节点")
    }
  }
  ...
  static func deleting(at path: [NBTPathComponent], in root: NBTValue) throws -> NBTValue {
    guard let last = path.last else {
      throw MCBEEditorError.unsupported("不能删除 NBT 根节点")
    }
    let parentPath = Array(path.dropLast())
    guard let parent = value(at: parentPath, in: root) else {
      throw MCBEEditorError.malformedData("NBT 父路径不存在")
    }
    let replacement: NBTValue
    switch (last, parent) {
    case (.compound(let name), .compound(var tags)):
      guard let index = tags.firstIndex(where: { $0.name == name }) else {
        throw MCBEEditorError.malformedData("NBT 标签不存在：\(name)")
      }
    ...
    default:
      throw MCBEEditorError.malformedData("NBT 删除路径与父节点类型不匹配")
    }
    return parentPath.isEmpty
      ? replacement : try replacingValue(at: parentPath, in: root, with: replacement)
  }

  static func renaming(at path: [NBTPathComponent], to newName: String, in root: NBTValue) throws
    -> NBTValue
  {
    guard case .compound(let oldName)? = path.last else {
      throw MCBEEditorError.unsupported("只有 Compound 标签可以重命名")
    }
    let cleanName = newName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !cleanName.isEmpty else {
      throw MCBEEditorError.malformedData("新名称不能为空")
    }
    let parentPath = Array(path.dropLast())
    guard case .compound(var tags)? = value(at: parentPath, in: root) else {
      throw MCBEEditorError.malformedData("NBT 父 Compound 不存在")
    }
    guard let index = tags.firstIndex(where: { $0.name == oldName }) else {
      throw MCBEEditorError.malformedData("NBT 标签不存在：\(oldName)")
    }
  ...
  static func defaultValue(for type: NBTTagType) throws -> NBTValue {
    ...
    case .end: throw MCBEEditorError.unsupported("不能创建 End 标签")
    }
  }
  ...
  static func parseInitialValue(_ text: String, type: NBTTagType) throws -> NBTValue {
    ...
    case .intArray:
      return .intArray(
        try parseIntegerList(text, range: Int64(Int32.min)...Int64(Int32.max)).map(Int32.init))
    case .longArray:
  ...
  private static func parseIntegerList(_ text: String, range: ClosedRange<Int64>) throws -> [Int64]
  {
    ...
    return try clean.split(whereSeparator: { $0 == "," || $0 == " " || $0 == ";" }).map { token in
      guard let value = Int64(token), range.contains(value) else {
        throw MCBEEditorError.malformedData("数组中包含无效整数：\(token)")
      }
      return value
    }
  }
  static func parseScalar(_ text: String, matching original: NBTValue) throws -> NBTValue {
    switch original {
    case .byte:
      guard let value = Int8(text) else { throw MCBEEditorError.malformedData("请输入 -128…127") }
      return .byte(value)
    case .short:
      guard let value = Int16(text) else { throw MCBEEditorError.malformedData("请输入 Int16 数值") }
      return .short(value)
    case .int:
      guard let value = Int32(text) else { throw MCBEEditorError.malformedData("请输入 Int32 数值") }
      return .int(value)
    case .long:
      guard let value = Int64(text) else { throw MCBEEditorError.malformedData("请输入 Int64 数值") }
      return .long(value)
    case .float:
      guard let value = Float(text) else { throw MCBEEditorError.malformedData("请输入浮点数") }
      return .float(value)
    case .double:
      guard let value = Double(text) else { throw MCBEEditorError.malformedData("请输入双精度浮点数") }
      return .double(value)
    case .string:
      return .string(text)
    default:
      throw MCBEEditorError.unsupported("当前节点不支持文本编辑")
    }
  }
}
