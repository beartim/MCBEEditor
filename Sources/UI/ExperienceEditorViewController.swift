...
final class ExperienceEditorViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, titleForFooterInSection section: Int) -> String? {
        "依次对应 experience 命令使用的 XpTotal、XpLevel 与 XpP。"
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.text = String(
            format: "UniqueID %@ · 总数 %d · 等级 %d · 进度 %.3f",
            uid,
...
private final class PlayerExperienceEditorViewController: UIViewController, UITextFieldDelegate {
    ...
    private func configureUI() {
        ...
        note.text = "经验总数范围为 -2147483648～2147483647；经验等级范围为 0～24791；经验条进度范围为 0～1。保存时直接写入 XpTotal、XpLevel、XpP。"
    ...
    @objc private func save() {
        ...
        guard let total64 = Int64(totalField.text ?? ""), let total = Int32(exactly: total64) else {
            showError(BlocktopographError.malformedData("经验总数必须在 Int32 范围内"), title: "经验错误")
            return
        }
        guard let level64 = Int64(levelField.text ?? ""), level64 >= 0, level64 <= Int64(BedrockPlayerExperience.maximumLevel) else {
