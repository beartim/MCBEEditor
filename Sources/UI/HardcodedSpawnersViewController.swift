...
final class HardcodedSpawnersViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        let dimension = BedrockDimension(rawValue: chunk.dimension)?.displayName ?? "维度 \(chunk.dimension)"
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Spawner")
    ...
    private func editArea(at index: Int) {
        ...
        openEditor(area: area) { [weak self] replacement in
            guard let self = self, var current = self.record,
                  current.document.areas.indices.contains(index) else { return }
    ...
    override func tableView(
        ...
        let action = UIContextualAction(style: .destructive, title: "删除") { [weak self] _, _, completion in
            guard let self = self, var current = self.record else { completion(false); return }
    ...
    private func openEditor(area: HardcodedSpawnerArea, completion: @escaping (HardcodedSpawnerArea) -> Void) {
        ...
        queue.async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    self.navigationItem.rightBarButtonItems?.first?.isEnabled = false
                    let message = record.document.areas.isEmpty
...
private final class HardcodedSpawnerAreaEditorViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "完成", style: .done, target: self, action: #selector(commit))

        let labels = ["最小 X", "最小 Y", "最小 Z", "最大 X", "最大 Y", "最大 Z"]
        let values = [area.minimumX, area.minimumY, area.minimumZ, area.maximumX, area.maximumY, area.maximumZ]
        var rows = [UIView]()
        ...
        let stack = UIStackView(arrangedSubviews: [labelled("刷怪类型", kindControl), labelled("自定义类型", customKindField)] + rows + [help])
        stack.axis = .vertical
        stack.spacing = 14
