...
private final class MapObjectOverlayView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        configureSelectionLayer(selectedVillageLayer, fill: UIColor.systemYellow.withAlphaComponent(0.12), dash: [10, 4])
        configureSelectionLayer(selectedSpawnerLayer, fill: UIColor.systemYellow.withAlphaComponent(0.16), dash: [6, 3])
        configureSelectionLayer(selectedObjectLayer, fill: .systemYellow, dash: nil)
        configureSelectionLayer(selectedBlockLayer, fill: UIColor.systemYellow.withAlphaComponent(0.28), dash: nil)
    ...
    func update(
        ...
        func clippedRect(minX: Int64, minZ: Int64, maxX: Int64, maxZ: Int64) -> CGRect? {
            ...
            guard maximumLocalX > 0, maximumLocalZ > 0,
                  minimumLocalX < CGFloat(sideBlocks), minimumLocalZ < CGFloat(sideBlocks) else { return nil }
        ...
        for hit in villageHits {
            ...
            if let center = feature.center {
                ...
                if bounds.insetBy(dx: -18, dy: -18).contains(p) {
                    ...
                    if hit.stableID == selectedVillageID, !hasSelectedVillagePath {
                        selectedVillagePath.append(UIBezierPath(ovalIn: CGRect(x: p.x - 11, y: p.y - 11, width: 22, height: 22)))
                        hasSelectedVillagePath = true
                    }
                }
            }
            for poi in feature.pointsOfInterest {
                ...
                if bounds.insetBy(dx: -12, dy: -12).contains(p) {
                    villagePOIPath.append(UIBezierPath(roundedRect: CGRect(x: p.x - 3.5, y: p.y - 3.5, width: 7, height: 7), cornerRadius: 1.2))
                }
            }
        }
        ...
        for link in villagePOILinks {
            ...
            guard bounds.insetBy(dx: -20, dy: -20).contains(source),
                  bounds.insetBy(dx: -20, dy: -20).contains(destination) else { continue }
            ...
            villagePOILinkPath.addLine(to: CGPoint(x: base.x + perpendicular.x * arrowWidth, y: base.y + perpendicular.y * arrowWidth))
            villagePOILinkPath.addLine(to: CGPoint(x: base.x - perpendicular.x * arrowWidth, y: base.y - perpendicular.y * arrowWidth))
        ...
        for hit in worldObjectHits {
            ...
            if hit.isNormallyVisible {
                if hit.object.kind == .entity {
                    entityPath.append(UIBezierPath(ovalIn: CGRect(x: center.x - 5.5, y: center.y - 5.5, width: 11, height: 11)))
                } else {
                    blockEntityPath.append(UIBezierPath(roundedRect: CGRect(x: center.x - 5, y: center.y - 5, width: 10, height: 10), cornerRadius: 1.5))
                }
            }
            if hit.object.stableID == selectedObjectID || selectedVillageEntityIDs.contains(hit.object.stableID) {
                hasSelectedPath = true
                if hit.object.kind == .entity {
                    selectedPath.append(UIBezierPath(ovalIn: CGRect(x: center.x - 8, y: center.y - 8, width: 16, height: 16)))
                } else {
                    selectedPath.append(UIBezierPath(roundedRect: CGRect(x: center.x - 7.5, y: center.y - 7.5, width: 15, height: 15), cornerRadius: 2.5))
                }
            }
        }
        ...
        for hit in spawnHits {
            ...
            case .world:
                worldSpawnPath.append(UIBezierPath(ovalIn: CGRect(x: center.x - 6.5, y: center.y - 6.5, width: 13, height: 13)))
            ...
            case .player:
                playerSpawnPath.append(UIBezierPath(ovalIn: CGRect(x: center.x - 6.5, y: center.y - 6.5, width: 13, height: 13)))
                playerSpawnGlyphPath.append(UIBezierPath(ovalIn: CGRect(x: center.x - 1.8, y: center.y - 3.6, width: 3.6, height: 3.6)))
                playerSpawnGlyphPath.move(to: CGPoint(x: center.x - 3.2, y: center.y + 3.5))
                playerSpawnGlyphPath.addQuadCurve(to: CGPoint(x: center.x + 3.2, y: center.y + 3.5), controlPoint: CGPoint(x: center.x, y: center.y - 0.4))
            }
        }
        ...
        if let chunk = selectedChunk, chunk.dimension == currentDimension {
            ...
            if localX + 16 > 0, localZ + 16 > 0, localX < CGFloat(sideBlocks), localZ < CGFloat(sideBlocks) {
                let topLeft = point(localX: localX, localZ: localZ)
        ...
        updateBlink(layer: selectedVillageLayer, key: "selected-village-blink", enabled: hasSelectedVillagePath, duration: 0.62)
        updateBlink(layer: selectedSpawnerLayer, key: "selected-spawner-blink", enabled: hasSelectedSpawnerPath, duration: 0.54)
        updateBlink(layer: selectedObjectLayer, key: "selected-object-blink", enabled: hasSelectedPath, duration: 0.48)
        updateBlink(layer: selectedBlockLayer, key: "selected-block-blink", enabled: hasSelectedBlockPath, duration: 0.42)
        updateBlink(layer: selectedChunkLayer, key: "selected-chunk-blink", enabled: hasSelectedChunkPath, duration: 0.58)
    }

    private func updateBlink(layer: CAShapeLayer, key: String, enabled: Bool, duration: CFTimeInterval) {
        if !enabled {
...
final class WorldMapViewController: UIViewController, UIScrollViewDelegate, UITextFieldDelegate, UIGestureRecognizerDelegate {
    ...
    private lazy var selectionMapPinchGesture: UIPinchGestureRecognizer = {
        let gesture = UIPinchGestureRecognizer(target: self, action: #selector(handleSelectionMapPinch(_:)))
        gesture.isEnabled = false
        gesture.delegate = self
        return gesture
    }()
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItems = [shareButton, overlayButton, selectionButton, zoomButton, cancelSelectionButton]
    ...
    @objc private func mapBlockSelectionRequested() {
        guard let coordinate = session.requestedMapBlockCoordinate,
              let dimensionIndex = BedrockDimension.allCases.firstIndex(where: { $0.rawValue == coordinate.dimension }) else { return }
        loadViewIfNeeded()
    ...
    private func configureUI() {
        ...
        coordinateModeControl.addTarget(self, action: #selector(coordinateModeChanged), for: .valueChanged)
        ...
        chunkSelectionSwitch.addTarget(self, action: #selector(chunkSelectionChanged), for: .valueChanged)
        ...
        let coordinates = UIStackView(arrangedSubviews: [displayOptions, flexibleSpacer, renderControls])
        ...
        let controls = UIStackView(arrangedSubviews: [coordinates, coordinateModeControl, dimensionControl, modeControl])
        ...
        statusLabel.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(showRenderDiagnostics)))
        ...
        let mapLongPress = UILongPressGestureRecognizer(target: self, action: #selector(handleChunkLongPress(_:)))
        ...
        let twoFingerTap = UITapGestureRecognizer(target: self, action: #selector(mapTwoFingerTapped(_:)))
        ...
        imageWidthConstraint = imageView.widthAnchor.constraint(equalToConstant: 80 * basePointsPerBlock)
        imageHeightConstraint = imageView.heightAnchor.constraint(equalToConstant: 80 * basePointsPerBlock)
        ...
        NSLayoutConstraint.activate([
            ...
            zoomLabel.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -12),
            zoomLabel.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -12),
    ...
    override func viewDidLayoutSubviews() {
        ...
        let preferredPanelWidth = traitCollection.horizontalSizeClass == .regular
            ? min(300, max(240, view.bounds.width * 0.28))
            : min(220, max(176, view.bounds.width * 0.42))
        if !blockDetailPanel.isCollapsed, abs(detailPanelWidthConstraint.constant - preferredPanelWidth) > 0.5 {
    ...
    private func setBlockDetailPanelCollapsed(_ collapsed: Bool, animated: Bool) {
        ...
        let expandedWidth = traitCollection.horizontalSizeClass == .regular
            ? min(300, max(240, view.bounds.width * 0.28))
            : min(220, max(176, view.bounds.width * 0.42))
    ...
    private func loadSpawn() {
        ...
        do {
            ...
            if let x = root.intValue(named: "SpawnX"), let z = root.intValue(named: "SpawnZ") {
                ...
                spawnCoordinates.append(MapSpawnCoordinate(
        ...
        do {
            ...
            for point in playerSpawns {
                spawnCoordinates.append(MapSpawnCoordinate(
                    stableID: "player:\(point.keyText):\(point.dimension):\(point.x):\(point.y ?? 0):\(point.z)",
                    kind: .player,
    ...
    @objc private func coordinateModeChanged() {
        updateCoordinateFields(centerX: lastCenterX, centerZ: lastCenterZ, anchor: currentViewportAnchor())
    ...
    private func render(
        ...
        renderQueue.async { [weak self] in
            ...
            do {
                ...
                let villageScan = includeVillages
                ...
                if includeEntities {
                    let entityScan = try scanner.scanRegion(
                        centerX: centerX,
                        centerZ: centerZ,
                        dimension: dimension,
                        radius: scanRadius,
                    ...
                    scannedObjects.append(contentsOf: entityScan.objects)
                    objectDiagnostics.append(contentsOf: entityScan.diagnostics)
                }
                ...
                if includeBlockEntities {
                    let blockEntityScan = try scanner.scanRegion(
                        centerX: centerX,
                        centerZ: centerZ,
                        dimension: dimension,
                        radius: scanRadius,
                    ...
                    scannedObjects.append(contentsOf: blockEntityScan.objects)
                    objectDiagnostics.append(contentsOf: blockEntityScan.diagnostics)
                }
                ...
                for object in scannedObjects {
                    if let current = uniqueObjects[object.stableID], current.source == .modernActor { continue }
                ...
                let result = try self.renderRegion(
                    ...
                    additionalErrors: objectDiagnostics + spawnerScan.diagnostics + villageScan.diagnostics + tickingDiagnostics,
                    shouldCancel: { token.isCancelled }
                )
                ...
                DispatchQueue.main.async {
                    ...
                    switch mode {
                    case .tickingAreas:
                        layerDetail = "常加载区域 \(result.tickingAreaCount) 个、定义区块 \(result.tickingDefinedChunkCount) 个、当前可见 \(result.visibleTickingChunkCount) 个；绿=普通、橙=预加载、紫=重叠"
                    case .slime:
    ...
    private func scanHardcodedSpawners(
        ...
        for dz in -leftChunks...rightChunks {
            for dx in -leftChunks...rightChunks {
                ...
                let key = BedrockDBKey(position: position, recordType: .hardcodedSpawners, subChunkIndex: nil).encoded()
                guard let raw = try database.get(key) else { continue }
                do {
                    let document = try HardcodedSpawnersDocument.decode(raw)
                    for (areaIndex, area) in document.areas.enumerated() {
                        hits.append(MapHardcodedSpawnerHit(
                            area: area,
                            ownerChunk: position,
                            areaIndex: areaIndex
                        ))
                    ...
                    diagnostics.append("HardcodedSpawners (\(position.x),\(position.z)): \(error.localizedDescription)")
                }
            }
        }
    ...
    private func entityScanRadius(
        ...
        let startBlockX = MapCoordinate.blockOrigin(ofChunk: centerX - Int32(renderRadius))
        let startBlockZ = MapCoordinate.blockOrigin(ofChunk: centerZ - Int32(renderRadius))
        let sideBlocks = Int64((renderRadius * 2 + 1) * 16)
        let endBlockX = startBlockX + sideBlocks - 1
        let endBlockZ = startBlockZ + sideBlocks - 1
        var requiredRadius = renderRadius

        func chunkDistance(_ value: Int32, from center: Int32) -> Int {
            Int(abs(Int64(value) - Int64(center)))
        }

        for village in villages where village.dimension == dimension {
            guard let bounds = village.bounds,
                  bounds.maximumX >= startBlockX, bounds.minimumX <= endBlockX,
                  bounds.maximumZ >= startBlockZ, bounds.minimumZ <= endBlockZ else { continue }
            let distances = [
                chunkDistance(MapCoordinate.chunk(fromBlock: bounds.minimumX), from: centerX),
                chunkDistance(MapCoordinate.chunk(fromBlock: bounds.maximumX), from: centerX),
                chunkDistance(MapCoordinate.chunk(fromBlock: bounds.minimumZ), from: centerZ),
                chunkDistance(MapCoordinate.chunk(fromBlock: bounds.maximumZ), from: centerZ)
            ]
            requiredRadius = max(requiredRadius, distances.max() ?? renderRadius)
        }
    ...
    private func renderRegion(
        ...
        let spawnHits = spawnCoordinates.compactMap { spawn -> MapSpawnHit? in
            guard spawn.x >= startBlockX, spawn.x < endBlockX,
                  spawn.z >= startBlockZ, spawn.z < endBlockZ else { return nil }
            return MapSpawnHit(
                spawn: spawn,
        ...
        let worldObjectHits = worldObjects.compactMap { object -> MapWorldObjectHit? in
            let isNormallyVisible = (object.kind == .entity && displayEntities) ||
                (object.kind == .blockEntity && displayBlockEntities)
            guard isNormallyVisible || villageResidentStableIDs.contains(object.stableID) else { return nil }
            guard let position = object.position,
                  position.blockX >= startBlockX, position.blockX < endBlockX,
                  position.blockZ >= startBlockZ, position.blockZ < endBlockZ else { return nil }
        ...
        let villageHits = villageFeatures.compactMap { feature -> MapVillageHit? in
            ...
            if let villageBounds = feature.bounds {
                guard villageBounds.maximumX >= startBlockX, villageBounds.minimumX < endBlockX,
                      villageBounds.maximumZ >= startBlockZ, villageBounds.minimumZ < endBlockZ else { return nil }
            } else if let center = feature.center {
                guard center.x >= startBlockX, center.x < endBlockX,
                      center.z >= startBlockZ, center.z < endBlockZ else { return nil }
            } else {
                guard feature.pointsOfInterest.contains(where: {
    ...
    private func drawSpawnMarker(context: CGContext, hit: MapSpawnHit) {
        ...
        context.saveGState()
        context.setShadow(offset: CGSize(width: 0, height: 0.45), blur: 0.65, color: UIColor.black.withAlphaComponent(0.55).cgColor)
        context.setFillColor((hit.spawn.kind == .world ? UIColor.systemYellow : UIColor.systemGreen).cgColor)
        context.fillEllipse(in: CGRect(x: x - radius, y: z - radius, width: radius * 2, height: radius * 2))
        context.setStrokeColor(UIColor.white.cgColor)
        context.setLineWidth(0.42)
        context.strokeEllipse(in: CGRect(x: x - radius, y: z - radius, width: radius * 2, height: radius * 2))
        context.setStrokeColor(UIColor.black.cgColor)
        context.setLineWidth(0.42)
        switch hit.spawn.kind {
        case .world:
            context.move(to: CGPoint(x: x - 0.75, y: z)); context.addLine(to: CGPoint(x: x + 0.75, y: z))
            context.move(to: CGPoint(x: x, y: z - 0.75)); context.addLine(to: CGPoint(x: x, y: z + 0.75))
        case .player:
            context.addEllipse(in: CGRect(x: x - 0.38, y: z - 0.82, width: 0.76, height: 0.76))
            context.move(to: CGPoint(x: x - 0.72, y: z + 0.78))
            context.addCurve(to: CGPoint(x: x + 0.72, y: z + 0.78), control1: CGPoint(x: x - 0.42, y: z - 0.10), control2: CGPoint(x: x + 0.42, y: z - 0.10))
        }
        context.strokePath()
        context.restoreGState()
    }

    private func drawWorldObject(context: CGContext, x: CGFloat, z: CGFloat, kind: BedrockWorldObjectKind) {
        let radius: CGFloat = kind == .entity ? 1.05 : 0.95
        context.saveGState()
        context.setShadow(offset: CGSize(width: 0, height: 0.35), blur: 0.5, color: UIColor.black.withAlphaComponent(0.55).cgColor)
    ...
    private func updateMapCanvasSize(sideBlocks: Int) {
        let side = max(basePointsPerBlock * 16, CGFloat(sideBlocks) * basePointsPerBlock)
    ...
    private func isVillageVillager(_ object: BedrockWorldObject) -> Bool {
        ...
        let identifier = object.identifier
            ...
            .lowercased()
        let localName = identifier.split(separator: ":", omittingEmptySubsequences: true).last.map(String.init) ?? identifier
    ...
    private func villagePOILinks(
        ...
        for villager in villagers {
            references[villager.object.stableID] = villageReferencePositionKeys(in: villager.object.document.root)
        }
        ...
        for village in villages {
            for point in village.feature.pointsOfInterest {
                ...
                for villager in villagers {
                    let idMatches = villager.object.uniqueID.map { point.linkedEntityIDs.contains($0) } ?? false
                    let positionKeys = references[villager.object.stableID] ?? []
                    let positionMatches = positionKeys.contains(exactKey) || positionKeys.contains(horizontalKey)
                    ...
                    result.append(MapVillagePOILink(
    ...
    private func villageReferencePositionKeys(in root: NBTValue) -> Set<String> {
        ...
        func walk(_ value: NBTValue, nameHint: String, inheritedRelevant: Bool) {
            ...
            case .compound(let tags):
                ...
                let isRelevant = inheritedRelevant || relationshipTerms.contains(where: { normalizedHint.contains($0) })
            ...
            case .list(_, let values):
                for child in values { walk(child, nameHint: nameHint, inheritedRelevant: inheritedRelevant) }
            default:
                break
            }
        }
    ...
    private func mapNBTVector(_ value: NBTValue) -> (Int64, Int64, Int64)? {
        ...
        case .list(_, let values) where values.count >= 3:
            guard let x = mapNBTInteger(values[0]),
                  ...
                  let z = mapNBTInteger(values[2]) else { return nil }
            return (x, y, z)
        case .compound(let tags):
    ...
    private func mapNBTXYZ(in tags: [NBTNamedTag]) -> (Int64, Int64, Int64)? {
        let x = tags.first(where: { ["x", "posx", "positionx", "blockx"].contains(normalizedNBTName($0.name)) })
            .flatMap { mapNBTInteger($0.value) }
        let y = tags.first(where: { ["y", "posy", "positiony", "blocky"].contains(normalizedNBTName($0.name)) })
            .flatMap { mapNBTInteger($0.value) }
        let z = tags.first(where: { ["z", "posz", "positionz", "blockz"].contains(normalizedNBTName($0.name)) })
            .flatMap { mapNBTInteger($0.value) }
    ...
    private func alignContentOffsetToDevicePixels() {
        ...
        if abs(aligned.x - scrollView.contentOffset.x) > 0.001 || abs(aligned.y - scrollView.contentOffset.y) > 0.001 {
    ...
    private func currentViewportAnchor() -> MapViewportAnchor? {
        ...
        let pointX = centerContentX / zoom
    ...
    private func applyViewport(anchor: MapViewportAnchor?) {
        ...
        view.layoutIfNeeded()
        let targetZoom = min(scrollView.maximumZoomScale, max(scrollView.minimumZoomScale, anchor?.zoomScale ?? 1))
        ...
        let localZ = (targetZ - Double(MapCoordinate.blockOrigin(ofChunk: startChunkZ))) / sideBlocks
        ...
        let maxOffsetZ = max(minOffsetZ, scrollView.contentSize.height - scrollView.bounds.height + scrollView.contentInset.bottom)
        let offset = CGPoint(
    ...
    private func scheduleAutoRender(immediate: Bool = false) {
        guard autoRenderSwitch.isOn, !isApplyingViewport, !isZooming, lastRenderedImage != nil else { return }
        panDebounceWorkItem?.cancel()
    ...
    private func mapPosition(at point: CGPoint) -> (localX: Int, localZ: Int, absoluteX: Int64, absoluteZ: Int64)? {
        ...
        return (
            localX,
            localZ,
            MapCoordinate.absoluteBlock(chunk: startChunkX, local: localX),
            MapCoordinate.absoluteBlock(chunk: startChunkZ, local: localZ)
        )
    }
    ...
    func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        if (gestureRecognizer === selectionPanGesture ||
            gestureRecognizer === selectionMapPanGesture ||
    ...
    @objc private func handleSelectionPan(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            selectionOverlayView.show(rect: CGRect(
    ...
    private func completeSelection(from start: CGPoint, to end: CGPoint) {
        ...
        updateSelectionOverlay(for: region)
        updateSelectionGestureAvailability()
        statusLabel.text = "已选择 \(region.coordinateText)，共 \(region.width)×\(region.depth) 方块；已禁止重新框选，可拖动四条边、输入坐标或移动缩放地图。"
    ...
    private func region(fromOverlayRect rawRect: CGRect) -> BedrockMapRegion? {
        ...
        let dimension = BedrockDimension.allCases[dimensionControl.selectedSegmentIndex].rawValue
        return BedrockMapRegion(minimumX: minX, minimumZ: minZ, maximumX: maxX, maximumZ: maxZ, dimension: dimension)
    }
    ...
    private func setSelectionCoordinates(x0: Int64, z0: Int64, x1: Int64, z1: Int64) {
        guard isSelectionMode else { return }
        let dimension = BedrockDimension.allCases[dimensionControl.selectedSegmentIndex].rawValue
        let region = BedrockMapRegion(minimumX: x0, minimumZ: z0, maximumX: x1, maximumZ: z1, dimension: dimension)
        let mapBounds = renderedMapRegion
        guard region.minimumX >= mapBounds.minimumX, region.maximumX <= mapBounds.maximumX,
              region.minimumZ >= mapBounds.minimumZ, region.maximumZ <= mapBounds.maximumZ else {
    ...
    private func adjustSelectionEdge(_ edge: MapSelectionEdge, translation: CGPoint, state: UIGestureRecognizer.State) {
    ...
    private func presentSelectionActions() {
        ...
        var message = "\(region.coordinateText)；\(region.width)×\(region.depth) 方块；涉及 \(region.chunkCount) 个区块。"
        ...
        alert.addAction(UIAlertAction(title: "复制区域内容到等大区域…", style: .default) { [weak self] _ in
            guard let self = self else { return }
            ...
            controller.onComplete = { [weak self] message, destination in
                self?.handleChunkMutation(
                    message: message,
                    preferredPosition: ChunkPosition(x: destination.minimumChunkX, z: destination.minimumChunkZ, dimension: destination.dimension)
                )
            }
            self.setSelectionMode(false)
            self.navigationController?.pushViewController(controller, animated: true)
        })
        alert.addAction(UIAlertAction(title: "区域内方块搜索替换…", style: .default) { [weak self] _ in
            guard let self = self else { return }
            let controller = ChunkSearchReplaceViewController(session: self.session, region: region)
            controller.onComplete = { [weak self] message in self?.handleChunkMutation(message: message, preferredPosition: nil) }
            self.setSelectionMode(false)
            self.navigationController?.pushViewController(controller, animated: true)
        })
        alert.addAction(UIAlertAction(title: "生物群系修改…", style: .default) { [weak self] _ in
            guard let self = self else { return }
        ...
        alert.addAction(UIAlertAction(title: "HardcodedSpawners 修改…", style: .default) { [weak self] _ in
            guard let self = self else { return }
            let controller = MapRegionHardcodedSpawnersViewController(session: self.session, region: region)
            controller.onMutation = { [weak self] message in self?.handleChunkMutation(message: message, preferredPosition: nil) }
            self.setSelectionMode(false)
            self.navigationController?.pushViewController(controller, animated: true)
        })
        alert.addAction(UIAlertAction(title: "清空区域…", style: .destructive) { [weak self] _ in
            self?.confirmRegionDestructiveAction(region: region, regenerate: false)
        })
        alert.addAction(UIAlertAction(title: "重新生成区域…", style: .destructive) { [weak self] _ in
        ...
        if let popover = alert.popoverPresentationController {
            ...
            popover.sourceRect = selectionOverlayView.selectionRect ?? CGRect(x: 8, y: 8, width: 1, height: 1)
        }
    ...
    private func scanAndShowSelectionObjects(region: BedrockMapRegion) {
        ...
        let radiusX = max(abs(Int(region.maximumChunkX - centerX)), abs(Int(centerX - region.minimumChunkX)))
        let radiusZ = max(abs(Int(region.maximumChunkZ - centerZ)), abs(Int(centerZ - region.minimumChunkZ)))
        ...
        chunkMenuQueue.async { [weak self] in
            ...
            do {
                ...
                let result = try scanner.scanRegion(
                    centerX: centerX,
                    centerZ: centerZ,
                ...
                let objects = result.objects.filter { object in
                    guard object.dimension == region.dimension, let position = object.position else { return false }
    ...
    private func confirmRegionDestructiveAction(region: BedrockMapRegion, regenerate: Bool) {
        ...
        let explanation = regenerate
            ? "将删除扩展范围内全部区块记录和关联 Actor，使 Minecraft 按种子重新生成。"
            : "将删除扩展范围内全部区块记录和关联 Actor，再写入已生成的纯空气区块。"
        let alert = UIAlertController(
            title: "\(actionName)区域？",
            message: "实际操作范围：\(aligned.coordinateText)，共 \(aligned.chunkCount) 个完整区块。\n\(explanation)此操作不会自动备份。",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        alert.addAction(UIAlertAction(title: actionName, style: .destructive) { [weak self] _ in
    ...
    private func performRegionDestructiveAction(region: BedrockMapRegion, regenerate: Bool) {
        ...
        chunkMenuQueue.async { [weak self] in
            ...
            do {
                ...
                let message = "已\(actionName) \(result.changedChunkCount) 个区块，跳过 \(result.skippedChunkCount) 个无记录区块。"
                DispatchQueue.main.async {
                    overlay.removeFromSuperview()
                    self.setSelectionMode(false)
    ...
    @objc private func mapTapped(_ recognizer: UITapGestureRecognizer) {
        ...
        if let hit = nearestWorldObject(localX: CGFloat(position.localX) + 0.5, localZ: CGFloat(position.localZ) + 0.5) {
    ...
    @objc private func handleChunkLongPress(_ recognizer: UILongPressGestureRecognizer) {
        ...
        if village != nil || spawner != nil {
            if let village = village, let spawner = spawner {
                ...
                alert.addAction(UIAlertAction(title: "编辑 \(village.feature.displayName)", style: .default) { [weak self] _ in
                    self?.openVillageEditor(village)
                })
                alert.addAction(UIAlertAction(title: "编辑 \(spawner.area.kind.displayName)", style: .default) { [weak self] _ in
    ...
    private func presentMapPointChoice(
        ...
        if let centerHit = centerHit {
            alert.addAction(UIAlertAction(title: "查看村庄中心", style: .default) { [weak self] _ in
    ...
    private func presentSpawnChoice(_ hits: [MapSpawnHit], sourcePoint: CGPoint) {
        let alert = UIAlertController(
            title: "选择出生点",
            message: "此位置附近有多个出生点。",
            preferredStyle: .actionSheet
        ...
        for hit in hits.prefix(12) {
            let title = hit.spawn.kind == .world
                ? "世界出生点"
                : "玩家出生点 · \(hit.spawn.name)"
            alert.addAction(UIAlertAction(title: title, style: .default) { [weak self] _ in
                self?.showSpawnInformation(hit.spawn)
            })
        }
    ...
    private func showSpawnInformation(_ spawn: MapSpawnCoordinate) {
        let dimension = BedrockDimension(rawValue: spawn.dimension)?.displayName ?? "维度 \(spawn.dimension)"
    ...
    private func showVillageCenterInformation(_ hit: MapVillageHit) {
        ...
        let message = "村庄中心坐标：X=\(center.x)，Y=\(center.y)，Z=\(center.z)\n玩家声望：\(reputationText)\n村民数目：\(hit.feature.villagerCount)"
        let alert = UIAlertController(title: hit.feature.displayName, message: message, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "关闭", style: .cancel))
        present(alert, animated: true)
        statusLabel.text = "已查看 \(hit.feature.displayName)；其 \(hit.feature.villagerEntities.count) 个村民和 \(hit.feature.ironGolemEntities.count) 个铁傀儡正在闪烁。"
    }
    ...
    @objc private func showChunkList() {
        let dimension = BedrockDimension.allCases[dimensionControl.selectedSegmentIndex].rawValue
        let controller = ChunkListViewController(session: session, initialDimension: dimension)
        controller.onSelectChunk = { [weak self] position in
            self?.selectChunk(position, centerMap: true)
        }
        controller.onSelectTickingArea = { [weak self] position in
            if let index = MapRenderMode.allCases.firstIndex(of: .tickingAreas) {
                self?.modeControl.selectedSegmentIndex = index
            }
            self?.selectChunk(position, centerMap: true)
        }
        controller.onChunkMutation = { [weak self] message, preferredPosition in
            self?.handleChunkMutation(message: message, preferredPosition: preferredPosition)
        }
        navigationController?.pushViewController(controller, animated: true)
    }
    ...
    private func selectChunk(_ position: ChunkPosition, centerMap: Bool) {
        ...
        blockDetailPanel.clearBlock()
        clearSelectedWorldObject()
        if let dimensionIndex = BedrockDimension.allCases.firstIndex(where: { $0.rawValue == position.dimension }) {
            dimensionControl.selectedSegmentIndex = dimensionIndex
        }
        ...
        if centerMap {
            ...
            render(
                centerX: position.x,
                centerZ: position.z,
                anchor: MapViewportAnchor(blockX: blockX, blockZ: blockZ, zoomScale: max(scrollView.zoomScale, scrollView.minimumZoomScale)),
                reason: "选择区块",
                showOverlay: true
            )
        } else {
    ...
    private func handleChunkMutation(message: String, preferredPosition: ChunkPosition?) {
        ...
        if let preferredPosition = preferredPosition {
            ...
            if let dimensionIndex = BedrockDimension.allCases.firstIndex(where: { $0.rawValue == preferredPosition.dimension }) {
                dimensionControl.selectedSegmentIndex = dimensionIndex
            }
        } else {
            selectedChunk = nil
    ...
    private func showBlockColumn(x: Int64, z: Int64, initialY: Int32?, annotation: String? = nil) {
        ...
        renderQueue.async { [weak self] in
            guard let self = self else { return }
            do {
                let result = try self.rendererForCurrentSession().blockColumn(blockX: x, blockZ: z, dimension: dimension)
                DispatchQueue.main.async {
                    overlay.removeFromSuperview()
                    let picker = BlockColumnPickerViewController(result: result, initialY: initialY) { [weak self] block in
    ...
    private func jumpToBlock(x: Int64, y: Int32, z: Int64) {
        ...
        renderQueue.async { [weak self] in
            guard let self = self else { return }
            do {
                let block = try self.rendererForCurrentSession().block(blockX: x, y: y, blockZ: z, dimension: dimension)
                DispatchQueue.main.async {
                    overlay.removeFromSuperview()
                    ...
                    self.render(
                        ...
                        anchor: MapViewportAnchor(blockX: Double(x) + 0.5, blockZ: Double(z) + 0.5, zoomScale: max(self.scrollView.zoomScale, 1)),
                        reason: "方块跳转",
                        showOverlay: true
                    )
                }
    ...
    private func nearestWorldObject(localX: CGFloat, localZ: CGFloat) -> MapWorldObjectHit? {
        let nearest = lastWorldObjectHits.filter(\.isNormallyVisible).min { lhs, rhs in
            worldObjectDistance(lhs, fromX: localX, z: localZ) < worldObjectDistance(rhs, fromX: localX, z: localZ)
        }
        guard let nearest = nearest, worldObjectDistance(nearest, fromX: localX, z: localZ) <= 1.8 else { return nil }
        return nearest
    }

    private func worldObjectDistance(_ hit: MapWorldObjectHit, fromX x: CGFloat, z: CGFloat) -> CGFloat {
        let dx = hit.localX - x
        let dz = hit.localZ - z
    ...
    private func showWorldObjectDetails(_ object: BedrockWorldObject) {
        ...
        alert.addAction(UIAlertAction(title: "定位到对象", style: .default) { [weak self] _ in self?.locate(worldObject: object) })
    ...
    func locate(worldObject: BedrockWorldObject) {
        ...
        render(
            ...
            anchor: MapViewportAnchor(blockX: position.x, blockZ: position.z, zoomScale: max(scrollView.zoomScale, 1)),
    ...
    @objc private func showOverlayOptions() {
        ...
        alert.addAction(UIAlertAction(title: blockTitle, style: .default) { [weak self] _ in
            guard let self = self else { return }
        ...
        alert.addAction(UIAlertAction(title: villageTitle, style: .default) { [weak self] _ in
            guard let self = self else { return }
    ...
    private func refreshObjectOverlays(reason: String) {
        saveMapState()
        let anchor = currentViewportAnchor()
        render(centerX: lastCenterX, centerZ: lastCenterZ, anchor: anchor, reason: reason, showOverlay: false)
    }

    @objc private func showZoomOptions() {
        let alert = UIAlertController(title: "地图缩放", message: "可直接双指捏合缩放；双击放大，两指轻点缩小。", preferredStyle: .actionSheet)
        alert.addAction(UIAlertAction(title: "适合屏幕", style: .default) { [weak self] _ in self?.setFitZoom(animated: true) })
        alert.addAction(UIAlertAction(title: "100%", style: .default) { [weak self] _ in self?.setZoomScale(1, animated: true) })
        alert.addAction(UIAlertAction(title: "200%", style: .default) { [weak self] _ in self?.setZoomScale(2, animated: true) })
        alert.addAction(UIAlertAction(title: "400%", style: .default) { [weak self] _ in self?.setZoomScale(4, animated: true) })
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
    ...
    private func showZoomHUD(autoHide: Bool = false) {
        zoomHUDWorkItem?.cancel()
        zoomLabel.text = String(format: "  %.0f%%  ", scrollView.zoomScale * 100)
    ...
    @objc private func showRenderDiagnostics() {
        ...
        alert.addAction(UIAlertAction(title: "清除区块缓存", style: .destructive) { [weak self] _ in
            guard let self = self else { return }
    ...
    private func drawVillagePOILinks(
        ...
        for link in links {
            ...
            context.beginPath()
            context.move(to: tip)
            context.addLine(to: CGPoint(x: base.x + perpendicular.x * 0.48, y: base.y + perpendicular.y * 0.48))
            context.addLine(to: CGPoint(x: base.x - perpendicular.x * 0.48, y: base.y - perpendicular.y * 0.48))
            context.closePath()
            context.fillPath()
        }
        ...
        return UIGraphicsImageRenderer(size: base.size, format: format).image { context in
            ...
            if layers.blockEntities {
                for hit in worldObjectHits where hit.isNormallyVisible && hit.object.kind == .blockEntity {
                    drawWorldObject(context: context.cgContext, x: hit.localX, z: hit.localZ, kind: .blockEntity)
                }
            }
            if layers.entities {
            ...
            do {
                ...
                case .currentRegion:
                    ...
                    if layers.entities || layers.blockEntities {
                        let scan = try BedrockWorldObjectScanner(database: database).scanRegion(
                            centerX: centerX,
                            centerZ: centerZ,
                            dimension: dimension,
                            radius: scanRadius,
                    ...
                    if layers.villages { objects.append(contentsOf: villageFeatures.flatMap(\.residentEntities)) }
                    let uniqueObjects = Dictionary(objects.map { ($0.stableID, $0) }, uniquingKeysWith: { current, candidate in
                        current.source == .modernActor ? current : candidate
                    }).map(\.value)
                    let spawnerScan = layers.hardcodedSpawners
                        ? try self.scanHardcodedSpawners(
                            database: database,
                            centerX: centerX,
                            centerZ: centerZ,
                ...
                case .loadedDimension:
                    ...
                    let spawnHits = selectedSpawns.compactMap { spawn -> MapSpawnHit? in
                        guard spawn.x >= base.startBlockX,
                              ...
                              spawn.z < base.startBlockZ + Int64(base.heightBlocks) else { return nil }
                        return MapSpawnHit(
                            spawn: spawn,
    ...
    private func renderLoadedDimensionBase(
        ...
        for position in positions.sorted(by: { lhs, rhs in
            ...
            let result = try renderer.renderChunk(x: position.x, z: position.z, dimension: position.dimension, mode: mode).result
    ...
    private func makeExportWorldObjectHits(
        ...
        return objects.compactMap { object in
            let normallyVisible = (object.kind == .entity && layers.entities) ||
                (object.kind == .blockEntity && layers.blockEntities)
            guard normallyVisible || residentIDs.contains(object.stableID), let position = object.position,
                  position.blockX >= startBlockX, position.blockX < endBlockX,
                  position.blockZ >= startBlockZ, position.blockZ < endBlockZ else { return nil }
            return MapWorldObjectHit(
                object: object,
        ...
        return features.compactMap { feature in
            if let bounds = feature.bounds {
                guard bounds.maximumX >= startBlockX, bounds.minimumX < endBlockX,
                      bounds.maximumZ >= startBlockZ, bounds.minimumZ < endBlockZ else { return nil }
            } else if let center = feature.center {
                guard center.x >= startBlockX, center.x < endBlockX,
                      center.z >= startBlockZ, center.z < endBlockZ else { return nil }
