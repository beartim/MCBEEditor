...
struct VillageNBTRecord: Hashable {
    ...
    static func == (lhs: VillageNBTRecord, rhs: VillageNBTRecord) -> Bool {
        lhs.key == rhs.key &&
        lhs.documentIndex == rhs.documentIndex &&
...
struct VillageMapFeature {
    ...
    var villagerEntities: [BedrockWorldObject] { residentEntities.filter { VillageNBTStore.residentKind(for: $0) == .villager } }
    var catEntities: [BedrockWorldObject] { residentEntities.filter { VillageNBTStore.residentKind(for: $0) == .cat } }
    var ironGolemEntities: [BedrockWorldObject] { residentEntities.filter { VillageNBTStore.residentKind(for: $0) == .ironGolem } }
...
final class VillageNBTStore {
    ...
    func scanRecords() throws -> VillageNBTScanResult {
            ...
            do {
                ...
                for (index, decodedRecord) in decoded.enumerated() {
                    if metadata.kind == .legacy {
                        ...
                        if split.isEmpty {
                            result.append(VillageNBTRecord(
                                key: entry.key,
                                keyText: keyText,
        ...
        result.sort { lhs, rhs in
            let villageOrder = lhs.villageIdentifier.localizedCaseInsensitiveCompare(rhs.villageIdentifier)
    ...
    func mapFeatures() throws -> VillageMapScanResult {
        ...
        for identifier in grouped.keys.sorted() {
            ...
            let primary = records.first(where: { $0.kind == .info })
            ...
            let dwellerUniqueIDs = Array(Set(
                dwellerRecords.flatMap { Self.dwellerUniqueIDs(
                    in: $0.document.root,
            ...
            if bounds == nil, !points.isEmpty {
                bounds = VillageMapBounds(
                    minimumX: points.map(\.x).min()! - 1,
                    minimumZ: points.map(\.z).min()! - 1,
                    maximumX: points.map(\.x).max()! + 1,
                    maximumZ: points.map(\.z).max()! + 1
                )
            }
    ...
    func residentResolution(villageIdentifier: String) throws -> VillageResidentResolution {
        ...
        let dwellerRecords = records.filter { $0.kind == .dwellers || $0.kind == .legacy }
        let uniqueIDs = Set(dwellerRecords.flatMap { record in
    ...
    private static func metadata(for key: Data, keyText: String) -> (identifier: String, kind: VillageNBTRecordKind) {
        ...
        for (suffix, kind) in suffixes where body.count >= suffix.count && body.suffix(suffix.count) == suffix {
    ...
    private static func legacyVillageRecords(
        ...
        let candidates = legacyCandidates(in: decodedRecord.document.root, path: [], nameHint: decodedRecord.document.rootName)
    ...
    private static func legacyCandidates(
        ...
        case .list(_, let values):
            ...
            let villageListNames: Set<String> = ["villages", "mvillages", "villagecollection", "villagelist"]
            let looksLikeVillageList = path.isEmpty || villageListNames.contains(normalizedHint)
            if looksLikeVillageList, !values.isEmpty, values.allSatisfy({ if case .compound = $0 { return true }; return false }) {
            ...
            for (index, child) in values.enumerated() {
                result.append(contentsOf: legacyCandidates(in: child, path: path + [.list(index)], nameHint: nameHint))
            }
        case .compound(let tags):
            for tag in tags {
                result.append(contentsOf: legacyCandidates(
                    in: tag.value,
    ...
    private static func identifier(in root: NBTValue) -> String? {
        ...
        for name in preferred {
            if let entry = values.first(where: { normalized($0.name) == name }), let text = scalarText(entry.value) {
                return text
            }
        }
        return nil
    ...
    private static func dimension(in root: NBTValue) -> Int32? {
        ...
        for entry in namedValues(in: root) where names.contains(normalized(entry.name)) {
            ...
            guard let text = scalarText(entry.value)?.trimmingCharacters(in: .whitespacesAndNewlines), !text.isEmpty else { continue }
            if let value = Int32(text) { return value }
            let value = normalized(text)
            if value == "overworld" || value.hasSuffix("minecraftoverworld") { return BedrockDimension.overworld.rawValue }
            if value == "nether" || value.hasSuffix("minecraftnether") { return BedrockDimension.nether.rawValue }
            if value == "end" || value == "theend" || value.hasSuffix("minecrafttheend") { return BedrockDimension.end.rawValue }
        }
        return nil
    }
    ...
    private static func center(in root: NBTValue) -> VillageMapPoint? {
        let centerNames = ["center", "villagecenter", "centerpos", "centerposition", "villagecenterpos"]
    ...
    private static func bounds(in root: NBTValue) -> VillageMapBounds? {
        ...
        for entry in entries {
            let name = normalized(entry.name)
            if ["min", "minimum", "minpos", "minimumposition", "lower", "lowercorner"].contains(name), let value = vector(entry.value) {
                minimum = value
            }
            if ["max", "maximum", "maxpos", "maximumposition", "upper", "uppercorner"].contains(name), let value = vector(entry.value) {
                maximum = value
            }
        }
    ...
    private static func collectPoints(
        ...
        case .compound(let tags):
            ...
            for tag in tags {
                ...
                if name.contains("pos") || name.contains("position") || name.contains("location") || name.contains("center") {
    ...
    private static func pointLabel(in tags: [NBTNamedTag]) -> String? {
        ...
        for name in preferred {
            if let tag = tags.first(where: { normalized($0.name) == name }), let text = scalarText(tag.value), !text.isEmpty {
                return text
            }
        }
        return nil
    ...
    private static func linkedEntityIDs(in tags: [NBTNamedTag]) -> [Int64] {
        let relationshipTerms = ["owner", "occupant", "dweller", "villager", "resident", "actor", "member"]
        var result = [Int64]()
        for tag in tags {
            let name = normalized(tag.name)
            let isRelationship = relationshipTerms.contains(where: { name.contains($0) })
            let isExplicitEntityID = ["actoruniqueid", "entityuniqueid", "owneruniqueid", "occupantuniqueid", "dwelleruniqueid", "villageruniqueid", "residentuniqueid"].contains(name)
        ...
        func walk(_ value: NBTValue, insideDwellers: Bool) {
            ...
            case .compound(let tags):
                for tag in tags {
                    ...
                    if childInsideDwellers && (name == "id" || explicitIdentityNames.contains(name) || name.hasSuffix("uniqueid")) {
    ...
    private static func dwellerEntityIDs(in root: NBTValue) -> [Int64] {
        let exactNames: Set<String> = [
            "actoruniqueid", "entityuniqueid", "dwelleruniqueid", "villageruniqueid",
            "residentuniqueid", "owneruniqueid", "occupantuniqueid", "actorid",
            "entityid", "dwellerid", "villagerid", "residentid"
        ]
        var result = [Int64]()
        func walk(_ value: NBTValue, parentName: String) {
            switch value {
            case .compound(let tags):
                for tag in tags {
                    let name = normalized(tag.name)
                    let relationshipName = ["dweller", "villager", "resident", "actor", "owner", "occupant"].contains(where: { name.contains($0) })
                    if exactNames.contains(name) || (relationshipName && name.contains("id")) {
                        result.append(contentsOf: numericValues(in: tag.value))
                    }
                    walk(tag.value, parentName: tag.name)
                }
            case .list(_, let values):
                let parent = normalized(parentName)
                if ["dwellers", "villagers", "residents", "actors", "members"].contains(where: { parent.contains($0) }) {
                    for child in values {
                        if let value = integer(child) { result.append(value) }
                    }
                }
                for child in values { walk(child, parentName: parentName) }
            default:
                break
            }
        }
        walk(root, parentName: "")
        return Array(Set(result)).sorted()
    }

    private static func villagerEntityIDs(in root: NBTValue) -> [Int64] {
        let identityNames: Set<String> = [
            "actoruniqueid", "entityuniqueid", "dwelleruniqueid", "villageruniqueid",
            "residentuniqueid", "actorid", "entityid", "dwellerid", "villagerid",
            "residentid", "uniqueid"
        ]
        let typeNames: Set<String> = [
            "identifier", "entityidentifier", "entitytype", "type", "id", "definition"
        ]
        var result = [Int64]()
        ...
        func walk(_ value: NBTValue, nameHint: String) {
            switch value {
            case .compound(let tags):
                let nameSaysVillager = normalized(nameHint).contains("villager")
                let typeSaysVillager = tags.contains { tag in
                    guard typeNames.contains(normalized(tag.name)), let text = scalarText(tag.value) else { return false }
                    return normalized(text).contains("villager")
                }
                if nameSaysVillager || typeSaysVillager {
                    if let implicitID = numericIdentifier(from: nameHint) { result.append(implicitID) }
                    for tag in tags where identityNames.contains(normalized(tag.name)) {
                        result.append(contentsOf: numericValues(in: tag.value))
                    }
                }
    ...
    private static func dwellerBindings(in root: NBTValue) -> [VillageDwellerBinding] {
        let identityNames: Set<String> = [
            "actoruniqueid", "entityuniqueid", "dwelleruniqueid", "villageruniqueid",
            "residentuniqueid", "actorid", "entityid", "dwellerid", "villagerid", "residentid", "uniqueid", "id"
        ]
        ...
        func walk(_ value: NBTValue, nameHint: String) {
            ...
            case .compound(let tags):
                ...
                for tag in tags {
                    ...
                    if positionTerms.contains(where: { name.contains($0) }), let coordinates = vector(tag.value) {
                        positions.append(VillageMapPoint(
                            x: coordinates.0,
                            y: coordinates.1,
                            z: coordinates.2,
                            label: tag.name
    ...
    private static func applying(bindings: [VillageDwellerBinding], to points: [VillageMapPoint]) -> [VillageMapPoint] {
        ...
        for binding in bindings {
            for position in binding.positions {
                if let index = result.firstIndex(where: { $0.coordinateKey == position.coordinateKey })
                    ?? result.firstIndex(where: { $0.horizontalCoordinateKey == position.horizontalCoordinateKey }) {
    ...
    private static func playerReputations(in root: NBTValue) -> [VillagePlayerReputation] {
        let reputationNames: Set<String> = ["reputation", "reputationvalue", "score", "popularity", "value"]
        let identityNames: Set<String> = ["playerid", "playeruniqueid", "uniqueid", "uuid", "xuid", "name", "playername", "id"]
        ...
        func walk(_ value: NBTValue, nameHint: String, indexHint: Int?) {
            ...
            case .compound(let tags):
                ...
                if let reputation = reputation {
                    let identity = tags.first(where: { identityNames.contains(normalized($0.name)) })
                ...
                if normalizedHint.contains("player") || normalizedHint.contains("reputation") {
                    for tag in tags {
                        ...
                        guard !reputationNames.contains(tagName), !identityNames.contains(tagName),
                              let value = integer(tag.value) else { continue }
    ...
    private static func deduplicatedReputations(_ values: [VillagePlayerReputation]) -> [VillagePlayerReputation] {
    ...
    private static func explicitVillagerCount(in root: NBTValue) -> Int? {
        let names: Set<String> = ["villagercount", "dwellercount", "residentcount", "population", "populationcount", "numvillagers"]
        for entry in namedValues(in: root) where names.contains(normalized(entry.name)) {
            if let value = integer(entry.value), value >= 0 { return Int(clamping: value) }
        }
        return nil
    }

    private static func estimatedDwellerCount(in root: NBTValue) -> Int {
        var best = 0
        func walk(_ value: NBTValue, nameHint: String) {
            switch value {
            case .list(_, let values):
                let name = normalized(nameHint)
                if ["dwellers", "villagers", "residents", "members"].contains(where: { name.contains($0) }) {
                    best = max(best, values.count)
                }
                for child in values { walk(child, nameHint: nameHint) }
            case .compound(let tags):
                for tag in tags { walk(tag.value, nameHint: tag.name) }
    ...
    private static func vector(_ value: NBTValue) -> (Int64, Int64, Int64)? {
        ...
        case .list(_, let values) where values.count >= 3:
            guard let x = integer(values[0]), let y = integer(values[1]), let z = integer(values[2]) else { return nil }
            return (x, y, z)
        case .compound(let tags):
    ...
    private static func xyz(in tags: [NBTNamedTag], prefixes: [String]) -> (Int64, Int64, Int64)? {
        for prefix in prefixes {
            ...
            let xNames = normalizedPrefix.isEmpty ? ["x"] : ["\(normalizedPrefix)x", "x\(normalizedPrefix)"]
            let yNames = normalizedPrefix.isEmpty ? ["y"] : ["\(normalizedPrefix)y", "y\(normalizedPrefix)"]
            let zNames = normalizedPrefix.isEmpty ? ["z"] : ["\(normalizedPrefix)z", "z\(normalizedPrefix)"]
            let x = tags.first(where: { xNames.contains(normalized($0.name)) }).flatMap { integer($0.value) }
            let y = tags.first(where: { yNames.contains(normalized($0.name)) }).flatMap { integer($0.value) }
            let z = tags.first(where: { zNames.contains(normalized($0.name)) }).flatMap { integer($0.value) }
    ...
    private static func scalar(namedAny names: [String], in tags: [NBTNamedTag]) -> Int64? {
        ...
        return tags.first(where: { normalizedNames.contains(normalized($0.name)) }).flatMap { integer($0.value) }
    }
    ...
    private static func deduplicated(_ points: [VillageMapPoint]) -> [VillageMapPoint] {
        ...
        for point in points {
            ...
            if let existing = merged[key] {
                let preferredLabel = existing.label == "兴趣点" && point.label != "兴趣点" ? point.label : existing.label
