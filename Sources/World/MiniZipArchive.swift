...
enum MiniZipArchive {
    ...
    static func create(from directory: URL, to archiveURL: URL) throws {
        ...
        while let fileURL = enumerator.nextObject() as? URL {
            ...
            var relative = String(standardized.dropFirst(rootPath.count + 1)).replacingOccurrences(of: "\\", with: "/")
    ...
    private static func parseCentralDirectory(_ archive: Data) throws -> [Entry] {
        ...
        for _ in 0..<entryCount {
            ...
            if flags & 0x0800 != 0 {
                ...
                path = String(data: nameData, encoding: .utf8) ?? String(data: nameData, encoding: .isoLatin1) ?? ""
            }
            entries.append(Entry(path: path, method: method, crc32: crc, compressedSize: compressedSize, uncompressedSize: uncompressedSize, localHeaderOffset: localOffset))
