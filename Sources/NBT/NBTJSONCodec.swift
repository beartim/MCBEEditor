...
enum NBTJSONCodec {
    ...
    private static func decodeTag(_ object: Any, path: String) throws -> NBTValue {
        guard let dictionary = object as? [String: Any], let rawType = dictionary["type"] as? String else {
    ...
    private static func decodePayload(type: NBTTagType, payload: Any, path: String) throws -> NBTValue {
        ...
        case .short:
            return .short(Int16(try integer(payload, path: path, minimum: Int64(Int16.min), maximum: Int64(Int16.max))))
        case .int:
            return .int(Int32(try integer(payload, path: path, minimum: Int64(Int32.min), maximum: Int64(Int32.max))))
        case .long:
        ...
        case .byteArray:
            ...
            return .byteArray(Data(try values.enumerated().map { index, item in
                UInt8(bitPattern: Int8(try integer(item, path: "\(path)[\(index)]", minimum: Int64(Int8.min), maximum: Int64(Int8.max))))
            }))
        case .string:
            guard let text = payload as? String else { throw typeError(path, expected: "字符串") }
            return .string(text)
        case .compound:
            guard let dictionary = payload as? [String: Any] else { throw typeError(path, expected: "对象") }
            let tags = try dictionary.keys.sorted().map { key in
                NBTNamedTag(name: key, value: try decodeTag(dictionary[key]!, path: "\(path).\(key)"))
            }
        ...
        case .intArray:
            guard let values = payload as? [Any] else { throw typeError(path, expected: "整数数组") }
            return .intArray(try values.enumerated().map { index, item in
                Int32(try integer(item, path: "\(path)[\(index)]", minimum: Int64(Int32.min), maximum: Int64(Int32.max)))
            })
        case .longArray:
            guard let values = payload as? [Any] else { throw typeError(path, expected: "长整数数组") }
            return .longArray(try values.enumerated().map { index, item in
    ...
    private static func inferValue(from object: Any, path: String) throws -> NBTValue {
        ...
        if let dictionary = object as? [String: Any] {
            return .compound(try dictionary.keys.sorted().map { key in
                NBTNamedTag(name: key, value: try inferValue(from: dictionary[key]!, path: "\(path).\(key)"))
            })
        }
    ...
    private static func integer(_ object: Any, path: String, minimum: Int64, maximum: Int64) throws -> Int64 {
        let value: Int64?
        if let text = object as? String { value = Int64(text) }
