...
final class BedrockWorldObjectScanner {
  private static let maximumDirectRegionChunkLookups = 262_144
  ...
  func scanRegion(
    ...
    let bounds = try Self.regionBounds(centerX: centerX, centerZ: centerZ, radius: radius)
    let chunkCount = try Self.chunkCount(in: bounds)
    guard chunkCount <= Self.maximumDirectRegionChunkLookups else {
      throw MCBEEditorError.unsupported(
        "实体扫描区域包含 \(chunkCount) 个区块，超过直接扫描安全上限；请改用自适应区域扫描。"
      )
    }

    outer: for chunkZValue in bounds.minimumZ...bounds.maximumZ {
      let chunkZ = Int32(chunkZValue)
      for chunkXValue in bounds.minimumX...bounds.maximumX {
        let chunkX = Int32(chunkXValue)
  ...
  /// Uses direct chunk-key lookups for small regions and switches to one
  /// indexed world scan for large regions. This prevents extreme map zoom or
  /// a very large selection from performing billions of missing-key reads.
  func scanRegionAdaptive(
    ...
    includeBlockEntities: Bool,
    maximumObjects: Int = 5_000,
    maximumDirectChunks: Int = 16_384,
    shouldCancel: () -> Bool = { false }
  ) throws -> BedrockWorldObjectScanResult {
    guard includeEntities || includeBlockEntities else { return .empty }
    let bounds = try Self.regionBounds(centerX: centerX, centerZ: centerZ, radius: radius)
    let chunkCount = try Self.chunkCount(in: bounds)
    if chunkCount <= max(1, maximumDirectChunks) {
      return try scanRegion(
        centerX: centerX,
        centerZ: centerZ,
        dimension: dimension,
        radius: radius,
    ...
    let worldScan = try scanAll(
      dimensions: Set([dimension]),
      includeEntities: includeEntities,
      includeBlockEntities: includeBlockEntities,
      maximumObjects: maximumObjects,
      objectFilter: { object in
        object.dimension == dimension
          && Int64(object.chunkX) >= bounds.minimumX
          && Int64(object.chunkX) <= bounds.maximumX
          && Int64(object.chunkZ) >= bounds.minimumZ
          && Int64(object.chunkZ) <= bounds.maximumZ
      },
      shouldCancel: shouldCancel
    )
    return BedrockWorldObjectScanResult(
      objects: worldScan.objects,
      diagnostics: ["扫描范围较大，已切换为世界索引扫描。"] + worldScan.diagnostics,
      actorDigestCount: worldScan.actorDigestCount,
      actorRecordCount: worldScan.actorRecordCount,
      legacyEntityRecordCount: worldScan.legacyEntityRecordCount,
      blockEntityRecordCount: worldScan.blockEntityRecordCount
    )
  }

  private static func regionBounds(
    centerX: Int32,
    centerZ: Int32,
    radius: Int
  ) throws -> (minimumX: Int64, maximumX: Int64, minimumZ: Int64, maximumZ: Int64) {
    guard radius >= 0 else { throw MCBEEditorError.malformedData("扫描半径不能为负数") }
    let radius64 = Int64(radius)
    return (
      max(Int64(Int32.min), Int64(centerX) - radius64),
      min(Int64(Int32.max), Int64(centerX) + radius64),
      max(Int64(Int32.min), Int64(centerZ) - radius64),
      min(Int64(Int32.max), Int64(centerZ) + radius64)
    )
  }

  private static func chunkCount(
    in bounds: (minimumX: Int64, maximumX: Int64, minimumZ: Int64, maximumZ: Int64)
  ) throws -> Int {
    let width = bounds.maximumX - bounds.minimumX + 1
    let depth = bounds.maximumZ - bounds.minimumZ + 1
    let (product, overflow) = width.multipliedReportingOverflow(by: depth)
    guard !overflow, product <= Int64(Int.max) else {
      throw MCBEEditorError.unsupported("实体扫描区域过大")
    }
    return Int(product)
  }
  ...
  func scanAll(
    ...
    objectFilter: ((BedrockWorldObject) -> Bool)? = nil,
    shouldCancel: () -> Bool = { false }
  ) throws -> BedrockWorldObjectScanResult {
    guard includeEntities || includeBlockEntities else { return .empty }
    ...
    func objectAllowed(_ object: BedrockWorldObject) -> Bool {
      dimensionAllowed(object.dimension) && (objectFilter?(object) ?? true)
    }
    ...
    if includeEntities {
      ...
      for entry in try database.entries(prefix: digestPrefix, includeValues: true, limit: 0) {
        ...
        if value.count % 8 != 0 {
          diagnostics.append(
            "实体摘要 (\(location.chunkX),\(location.chunkZ)) 长度 \(value.count) 不是 8 的倍数。")
        }
    ...
    if includeBlockEntities || includeEntities {
      ...
      for entry in try database.entries(includeValues: false, limit: 0) {
        ...
        guard entry.key.count == 9 || entry.key.count == 13,
          ...
          dimensionAllowed(parsed.position.dimension)
        else { continue }
        ...
        do {
          ...
          diagnostics.append(
            "\(label) (\(parsed.position.x),\(parsed.position.z))：\(error.localizedDescription)")
        }
      }
    }
  ...
  func scanEntities(
    ...
    if !unresolved.isEmpty && !shouldCancel() {
      ...
      for entry in entries {
        ...
        do {
          ...
          diagnostics.append(
            "旧实体 \(parsed.position.x),\(parsed.position.z)：\(error.localizedDescription)")
        }
      }
    }
    ...
    if !unresolvedIDs.isEmpty {
      diagnostics.append(
        "未在世界实体中找到 Dwellers ID（实体 UniqueID）：\(unresolvedIDs.map(String.init).joined(separator: ", "))"
      )
    }
  ...
  private func makeObject(
    ...
    if let uniqueID = uniqueID {
      ...
      stableID =
        "\(kind.rawValue):\(resolvedDimension):\(position.blockX):\(position.blockY):\(position.blockZ):\(identifier):\(index)"
    } else {
      stableID =
        "\(kind.rawValue):\(resolvedDimension):\(fallbackChunkX):\(fallbackChunkZ):\(identifier):\(index)"
    }
  ...
  private func resolveIdentifier(root: NBTValue, kind: BedrockWorldObjectKind) -> String {
    ...
    if kind == .entity {
      ...
      if let numericID = root.int64Value(
        namedAny: primaryNames + ["EntityType", "entityType", "EntityID", "entityID"])
      {
        if let entry = BedrockDataValueCatalog.entity(forNumericID: numericID) {
          return entry.identifier
        }
  ...
  private func firstEntityDefinitionIdentifier(root: NBTValue) -> String? {
    guard let first = root.value(namedAny: ["definitions", "Definitions"])?.listValues?.first,
      case .string(let rawDefinition) = first
    else {
      return nil
    }
  ...
  private func extractDimension(root: NBTValue) -> Int32? {
    guard
      let value = root.int64Value(namedAny: [
        "DimensionId", "DimensionID", "dimensionId", "dimension", "Dimension",
      ])
    else {
      return nil
    }
